...
void MoveBallRight(void) {
	...
	while (1) {
		...
		if (XScuTimer_IsExpired(TimerInstancePtr)) {
			...
			if(psb_check == LED_PATTERNS[3] && (count !=3)){
				EarlyPress = 1;
			}else if ((psb_check & LED_PATTERNS[3]) == LED_PATTERNS[count]) {
				EarlyPress = 0;
			} else {
				EarlyPress = 1;
			}
	...
	if (EarlyPress == 1) {
		...
		xil_printf(" Game stop Score Left = %d   Score Right = %d\r\n",
				scoreleft,scoreright);
		GameOver = STOP;
		EarlyPress=0;
	} else {
...
void MoveBallLeft(void) {
	...
	while (1) {
		...
		if (XScuTimer_IsExpired(TimerInstancePtr)) {
			...
			LED_IP_mWriteReg(XPAR_LED_IP_S_AXI_BASEADDR, 0,
					LED_PATTERNS[count]);
			if(psb_check == LED_PATTERNS[0] && (count !=0)){
				EarlyPress = 1;
			}else if ((psb_check & LED_PATTERNS[0]) == LED_PATTERNS[count]) {
				EarlyPress = 0;
			} else {
				EarlyPress = 1;
			}
