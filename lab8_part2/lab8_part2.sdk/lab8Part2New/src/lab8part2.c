...
int main(void) {
	...
	XGpio_Initialize(&uart1, XPAR_AXI_GPIO_0_DEVICE_ID);
			...
			XGpio_Initialize(&uart_tx_full, XPAR_AXI_GPIO_1_DEVICE_ID);
...
//	/* Wait until bus is idle to start another transfer. */
	...
	sprintf(Buffer, "\r\n                             Current Temperature is: %6.3f Celsius", tempA);
	print_message(Buffer);
	printf("                                          Current Temperature is: %6.3f  Fahrenheit\r\n", tempAF);
	sprintf(Buffer, "\r\n                             Current Temperature is: %6.3f  Fahrenheit\r\n", tempAF);
	print_message(Buffer);
...
//	/* Wait until bus is idle to start another transfer. */
	...
	xil_printf("TMP101 B \r\n");
	printf("                              Current Temperature is: %6.3f celsius \r\n", tempB);
	sprintf(Buffer, "\r\n                 Current Temperature is: %6.3f celsius ", tempB);
	print_message(Buffer);
	printf("                              Current Temperature is: %6.3f fahrenheit\r\n", tempBF);
	sprintf(Buffer, "\r\n                 Current Temperature is: %6.3f fahrenheit", tempBF);
		print_message(Buffer);
	return XST_SUCCESS;
}
