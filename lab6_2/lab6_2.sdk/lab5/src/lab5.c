...
int main(void) {
	...
	while (1) {
		...
		if ((psb_check & 0b0100) == RESETBUTTON)	//reset game
				{
		...
		if ((psb_check & 0b0010) == STARTBUTTON) {
...
void MoveBallRight(void) {
	...
	while (1) {
		...
		if (XScuTimer_IsExpired(TimerInstancePtr)) {
			...
			LED_IP_mWriteReg(XPAR_LED_IP_S_AXI_BASEADDR, 0,
					LED_PATTERNS[count]);
			if ((psb_check & 0b0001) == LED_PATTERNS[count]) {
				EarlyPress = 0;
			} else {
				EarlyPress = 1;
			}
	...
	if (EarlyPress == 1) {
		leftResult = scoreleft + 1;
		rightResult = scoreright;
//		scoreright = 0;
//		scoreright = 0;
...
void MoveBallLeft(void) {
	...
	// check for button pushes
	if (EarlyPress == 1) {
		leftResult = scoreleft;
		rightResult = scoreright + 1;
//		scoreright = 0;
//		scoreright = 0;
		xil_printf("Game Over! Score Left = %d   Score Right = %d\r\n",
				leftResult, rightResult);
